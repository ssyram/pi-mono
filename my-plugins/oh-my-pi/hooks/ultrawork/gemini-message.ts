...
**WHEN IN DOUBT:**
\`\`\`
call_agent(agent="explore", skills=[], prompt="I'm implementing [TASK DESCRIPTION] and need to understand [SPECIFIC KNOWLEDGE GAP]. Find [X] patterns in the codebase — show file paths, implementation approach, and conventions used. I'll use this to [HOW RESULTS WILL BE USED]. Focus on src/ directories, skip test files unless test patterns are specifically needed. Return concrete file paths with brief descriptions of what each file does.", background=true)
call_agent(agent="librarian", skills=[], prompt="I'm working with [LIBRARY/TECHNOLOGY] and need [SPECIFIC INFORMATION]. Find official documentation and production-quality examples for [Y] — specifically: API reference, configuration options, recommended patterns, and common pitfalls. Skip beginner tutorials. I'll use this to [DECISION THIS WILL INFORM].", background=true)
call_agent(agent="oracle", skills=[], prompt="I need architectural review of my approach to [TASK]. Here's my plan: [DESCRIBE PLAN WITH SPECIFIC FILES AND CHANGES]. My concerns are: [LIST SPECIFIC UNCERTAINTIES]. Please evaluate: correctness of approach, potential issues I'm missing, and whether a better alternative exists.", background=false)
\`\`\`
...
\`\`\`
call_agent(agent="prometheus", skills=[], prompt="<gathered context + user request>")
\`\`\`
...
| Scenario | Action |
|----------|--------|
| Plan agent asks clarifying questions | \`delegate_task(session_id="{returned_session_id}", skills=[], prompt="<your answer>")\` |
| Need to refine the plan | \`delegate_task(session_id="{returned_session_id}", skills=[], prompt="Please adjust: <feedback>")\` |
| Plan needs more detail | \`delegate_task(session_id="{returned_session_id}", skills=[], prompt="Add more detail to Task N")\` |
...
|-----------|--------|-----|
| Codebase exploration | call_agent(agent="explore", skills=[], background=true) | Parallel, context-efficient |
| Documentation lookup | call_agent(agent="librarian", skills=[], background=true) | Specialized knowledge |
| Planning | call_agent(agent="prometheus", skills=[]) | Parallel task graph + structured TODO list |
| Hard problem (conventional) | call_agent(agent="oracle", skills=[]) | Architecture, debugging, complex logic |
| Hard problem (non-conventional) | delegate_task(category="artistry", skills=[...]) | Different approach needed |
| Implementation | delegate_task(category="...", skills=[...]) | Domain-optimized models |
...
2. Spawn exploration/librarian agents via delegate_task(background=true) in PARALLEL
