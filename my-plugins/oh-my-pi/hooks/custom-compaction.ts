 ...
 * 8-section prompt ported from oh-my-openagent's compaction-context-injector.
 *
...
import type { ConcurrencyManager } from "../tools/concurrency.js";
...
- For each: agent name, status, description
- **RESUME, DON'T RESTART.** Each listed session retains full context. After compaction, reuse existing agent sessions instead of spawning new ones. This saves tokens, preserves learned context, and prevents duplicate work.
...
const UPDATE_COMPACTION_PROMPT =
  ...
  "- Follow the same 8-section structure\n\n" +
  COMPACTION_PROMPT;
...
function formatJobContext(concurrency: ConcurrencyManager): string {
  ...
  const lines = jobs
    .map(
      (j) =>
        `- [${j.id}] agent=${j.agent} status=${j.status} task="${j.task}"`,
    )
    .join("\n");
  ...
  pi.on(
    ...
    async (event: SessionBeforeCompactEvent, ctx) => {
      ...
      try {
        ...
        const jobContext = formatJobContext(concurrency);
