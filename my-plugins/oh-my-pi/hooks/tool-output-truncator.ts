 ...
 *
 * Intercepts every `tool_result` event and, when the total text exceeds
 * `MAX_OUTPUT_CHARS`, truncates text blocks proportionally while keeping
 * non-text blocks (images) intact.
 *
 * Orthogonal to the Impression System: this hook runs at the raw
 * tool-result level; impressions operate on the conversation context.
 */
