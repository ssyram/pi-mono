 ...
 *
 * Flow:
 * 1. Accept plan text as args, or fall back to extracting recent plan from session
 * 2. Create ephemeral Momus session with read-only tools
 * 3. Prompt Momus to review the plan
 * 4. Parse for OKAY/REJECT verdict
 * 5. Inject result into main context
 * 6. Dispose session
 */
...
export function registerReviewPlan(
	...
	pi.registerCommand("omp-review", {
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			try {
				await handleReview(pi, agents, args.trim(), ctx);
			} catch (err: unknown) {
