...
export const sisyphus: AgentDef = {
  name: "sisyphus",
...
Fire explore/librarian agents in background, then continue only with non-overlapping work. If none exists, end your response and wait for completion notification.
...
4. Collect via \`background_task({ action: "status", jobId: "..." })\`
...
When delegating implementation tasks, select the appropriate category for domain-optimized execution.

#### Available Categories (Domain-Optimized)

Each category runs with a configuration optimized for that domain. Match your task to the right category:
