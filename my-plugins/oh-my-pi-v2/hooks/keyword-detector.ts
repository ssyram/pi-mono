...
AGENT DISPATCH:
\`\`\`
subagent({agent: "explore", task: "Find all occurrences of [PATTERN] in the codebase — file paths, usage patterns, and surrounding context. Be exhaustive. Search src/, packages/, and config files."})
subagent({agent: "librarian", task: "Find official documentation and real-world examples for [TOPIC]. Include API references, configuration guides, and common patterns from production codebases."})
\`\`\`
...
\`\`\`
subagent({agent: "explore", task: "Find all implementations related to [TOPIC] — show file paths, architecture patterns, data flow, and dependencies. Map the full dependency graph."})
subagent({agent: "librarian", task: "Find authoritative references for [TECHNOLOGY/CONCEPT]. Include architecture guides, best practices, and known pitfalls from official docs."})
\`\`\`

IF COMPLEX - DO NOT STRUGGLE ALONE. Consult specialists:
- **Oracle**: Conventional problems (architecture, debugging, complex logic)
  \`subagent({agent: "oracle", task: "Analyze [PROBLEM]. Evaluate approach, identify risks, suggest alternatives."})\`
- **Artistry**: Non-conventional problems (different approach needed)
  \`subagent({agent: "artistry", task: "This problem needs a creative approach: [DESCRIBE CONSTRAINTS]."})\`
