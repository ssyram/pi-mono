...
|----------|-------------|------------|
| explore agent | Need codebase patterns you don't have | \`call_agent(agent="explore", skills=[], background=true, ...)\` |
| librarian agent | External library docs, OSS examples | \`call_agent(agent="librarian", skills=[], background=true, ...)\` |
| oracle agent | Stuck on architecture/debugging after 2+ attempts | \`call_agent(agent="oracle", skills=[], ...)\` |
| plan agent | Complex multi-step with dependencies (5+ steps) | \`call_agent(agent="prometheus", skills=[], ...)\` |
| task category | Specialized work matching a category | \`delegate_task(category="...", skills=[...])\` |
...
call_agent(agent="explore", skills=[], prompt="I'm implementing [TASK] and need to understand [KNOWLEDGE GAP]. Find [X] patterns in the codebase \u2014 file paths, implementation approach, conventions used, and how modules connect. I'll use this to [DOWNSTREAM DECISION]. Focus on production code in src/. Return file paths with brief descriptions.", background=true)
call_agent(agent="librarian", skills=[], prompt="I'm working with [TECHNOLOGY] and need [SPECIFIC INFO]. Find official docs and production examples for [Y] \u2014 API reference, configuration, recommended patterns, and pitfalls. Skip tutorials. I'll use this to [DECISION THIS INFORMS].", background=true)
