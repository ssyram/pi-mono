...
export function registerStartWork(
	...
	pi.registerCommand("omp-start", {
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			...
			const prometheusPrompt = await readAgentPrompt(agentsDir, "prometheus");
			const momusPrompt = await readAgentPrompt(agentsDir, "momus");
			if (!prometheusPrompt || !momusPrompt) {
			...
			try {
				...
				try {
					session.agent.state.systemPrompt = prometheusPrompt;
					...
					for (let round = 0; round < MAX_INTERVIEW_ROUNDS; round++) {
						...
						if (isPlanReady(response)) {
							...
							const reviewResult = await runMomusReview(ctx, momusPrompt, response);
