...
- `execute.ts`: `executeTaskRequest(previous: TaskState, input: unknown): TaskOperation`. Returns a cloned complete candidate, `changed`, and the native-shaped result. Business/request errors return `details.error`; batch skips and successful `done` handoff start failures return structured outcomes rather than whole-call failure. This pure transform is still fallible; harness callers use the containing session methods, not this low-level export directly.
- `start-next.ts`: internal `executeStartNext(startNext, tasks, nextId): TaskOutcome[]`. It normalizes omitted/scalar/array handoff input, invokes existing `executeStart` in supplied order against the candidate, and maps the existing result to `started` or `start_skipped`. It is not a second start validator or a public lifecycle API.
...
At the new `pi.registerTool` call in `tools/task.ts`, retain `renderTaskCall` if desired but omit the old `renderTaskResult` callback. The native default result renderer displays the already-computed result text, including partial application and satisfied-terminal-edge notices. The retained call renderer displays a `done` handoff as `startNext=[...]`, normalizing a scalar for display only; result text remains authoritative for actual outcomes.
